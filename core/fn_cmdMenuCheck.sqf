...
while {true} do
{
	waitUntil {uiSleep 0.1;commandingMenu != _lastMenu};
	_lastMenu = commandingMenu;

	if(!(commandingMenu in _validMenus)) then {
		[[profileName,getPlayerUID player,_lastMenu],"OEC_fnc_observe",false,false] call OEC_fnc_MP;
		// [[[0,1],format["%1 was observed by SPY-GLASS, he/she was trying to access commanding menu:\n\n %2\n\n and that commanding-menu is not known to the system. PLEASE NOTE he/she may not be cheating but the SPY-GLASS found it relavent to report in",profileName,_lastMenu]],"OEC_fnc_broadcast",-2,false] spawn OEC_fnc_MP;
		showCommandingMenu "";
	};
};
