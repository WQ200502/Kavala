...
switch (_index) do {
	case 0:	{
		_config = [_shop] call OEC_fnc_weaponShopCfgAltis;

		{
			...
			_itemList lbSetValue[(lbSize _itemList)-1,(((_x select 2) * __GETC__(life_medDis)) * _goalDis)];
	...
	case 1:	{
		...
		((findDisplay 38400) displayCtrl 38405) ctrlSetText localize "STR_Global_Sell";
		{
			if(!(_x in _listedItems) && _x != "") then {
				_itemInfo = [_x] call OEC_fnc_fetchCfgDetails;
