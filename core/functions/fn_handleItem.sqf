...
//  Description: Main gear handling functionality.
...
_preview = param [7,false,[true]];

//Some checks
if(_item == "") exitWith {};
_isgun = false;

_details = [_item] call OEC_fnc_fetchCfgDetails;
	...
	switch((_details select 6)) do {
		...
		case "CfgVehicles": {
			...
			if(!isNil {_items}) then {
				{
					[_x,true,true,false,true] spawn OEC_fnc_handleItem;
				} foreach _items;
			};
		};
				...
				switch(_details select 5) do {
						...
						if(_ispack) then {
							...
							if(_override) then {
								...
								if(_item in (assignedItems player)) then {
									...
									if (_item in ["ItemMap", "ItemGPS", "ItemWatch", "ItemRadio", "ItemCompass"]) then {
						...
						if(_ispack) then {
							...
							if(_override) then {
								...
								if(vest player == _item) then {
									...
									if(!isNil {_items}) then {
										{
											[_x,true,false,false,true] spawn OEC_fnc_handleItem;
										} foreach _items;
									};
								};
							};
					...
					case 201: {
						if(_ispack) then {
							...
							private["_type"];
							_type = [_item,201] call OEC_fnc_accType;
							if(_ongun) then {
								switch (_type) do {
								...
								if(_override) then {
									...
									_wepItems = switch(_type) do {
										case 1: {primaryWeaponItems player};
										case 2: {secondaryWeaponItems player};
										case 3: {handgunItems player};
										default {["","",""]};
									};
									_slotTaken = false;
					...
					case 301: {
						if(_ispack) then {
							...
							private["_type"];
							_type = [_item,301] call OEC_fnc_accType;
					...
					case 101: {
						if(_ispack) then {
							...
							private["_type"];
							_type = [_item,101] call OEC_fnc_accType;
