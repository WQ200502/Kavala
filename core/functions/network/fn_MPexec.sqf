...
//	Parameter(s):
//	_this select 0: STRING - Packet variable name (always "OEC_fnc_MP_packet")
//	_this select 1: ARRAY - Packet value (sent by OEC_fnc_MP function; see it's description for more details)
...
if(!(["OEC_fnc_",_functionName] call BIS_fnc_inString) && {!(["OEC_fnc_",_functionName] call BIS_fnc_inString)} && {!(["OES_fnc_",_functionName] call BIS_fnc_inString)} && {!(["OES_fnc_",_functionName] call BIS_fnc_inString)} &&
{!(toLower(_functionName) in ["bis_fnc_execvm","bis_fnc_effectkilledairdestruction","bis_fnc_effectkilledairdestructionstage2"])} && {!(["OEC_fnc_",_functionName] call BIS_fnc_inString)}) exitWith {false};
if(toLower(_functionName) == "OES_fnc_asynccall") exitWith {false};
if(toLower(_functionName) == "OES_fnc_mresToArray") exitWith {false};
...
if(_callerUID != "__SERVER__" && _callerName != "__SERVER__" && toLower(_functionName) in ["OEC_fnc_cookiejar","OEC_fnc_notifyadmins"]) then {
	//Check if the sender & reported UID match, if they don't exit.
	if(toLower(_functionName) == "OEC_fnc_cookiejar") exitWith {
		private["_reportUID"];
		_reportUID = _params select 1;
		if(_reportUID != _callerUID) exitWith {
			if(isServer && _mode == 0) then {
				[_callerName,_callerUID,"false_reports_to_spyglass"] call OEC_fnc_cookieJar;
				[[_callerName,"False reporting to SpyGlass (cheater)"],"OEC_fnc_notifyAdmins",-2,false] spawn OEC_fnc_MP;
			};
			_exitScope = true;
		};
	};
...
if (ismultiplayer && _mode == 0) then {
	if (isserver) then {
		if (_target isEqualType []) then {
			...
			//--- Multi execution
			{
				[_varName,[_mode,_params,_functionName,_x,_isPersistent,_isCall,_callerName,_callerUID]] call OEC_fnc_MPexec;
			} foreach _target;
		} else {
			...
			OEC_fnc_MP_packet = [1,_params,_functionName,_target,_isPersistent,_isCall,"__SERVER__","__SERVER__"];

			//--- Send to clients
			if (_ownerID < 0) then {
				//--- Everyone
				publicvariable "OEC_fnc_MP_packet";
			} else {
				if (_ownerID != _serverID) then {
					//--- Client
					_ownerID publicvariableclient "OEC_fnc_MP_packet";
				};
			};

			//--- Server execution (for all or server only)
			if (_ownerID < 0 || _ownerID == _serverID) then {
				["OEC_fnc_MP_packet",OEC_fnc_MP_packet] spawn OEC_fnc_MPexec;
			};
