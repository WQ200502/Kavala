...
//	Description: Updates players crime list
...
if(_crimeNumber isEqualTo 999) exitWith {
	O_stats_crimes = [];
...
O_stats_crimes set[0,_currentBounty + _crime];
O_stats_crimes set[_crimeNumber, _selectedCrime + 1];

player setVariable["statBounty",O_stats_crimes select 0,true];
if (_crimeNumber == 58) then {
	oev_dispatchCooldown = true;
	profileNamespace setVariable["oev_dispatchCooldown",true];
	systemChat "由于系统使用不当，您无法向APD紧急服务部门发送短信！";
};
[10] call OEC_fnc_ClupdatePartial;
[] call OEC_fnc_hudUpdate;
